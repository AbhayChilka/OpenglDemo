...
{
	GLfloat x_left, x_right, y_bottom, y_top, z_near, z_far;
	// code
	if (height <= 0)
		height = 1;
	...
	ortho_Matrix[0] = 2.0f / (x_right - x_left);
	ortho_Matrix[1] = 0;
	ortho_Matrix[2] = 0;
	ortho_Matrix[3] = 0;

	ortho_Matrix[4] = 0;
	ortho_Matrix[5] = 2.0f / (y_top - y_bottom);
	ortho_Matrix[6] = 0;
	ortho_Matrix[7] = 0;

	ortho_Matrix[8] = 0;
	ortho_Matrix[9] = 0;
	ortho_Matrix[10] = (-2.0f) / (z_far - z_near);
	ortho_Matrix[11] = 0;

	ortho_Matrix[12] = -(x_right + x_left) / (x_right - x_left);
	ortho_Matrix[13] = -(y_top + y_bottom) / (y_top - y_bottom);
	ortho_Matrix[14] = -(z_far + z_near) / (z_far - z_near);
	ortho_Matrix[15] = 1;
