...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    PlaySound("deathlyhallowssong.wav", NULL, SND_ASYNC| SND_FILENAME);
...
void display(void)
{
	...
	glTranslatef(t_x, t_y, -3.7f);
	glRotatef(tangle, 0.0f, 1.0f, 0.0f);
	Triangle();
	glLoadIdentity();

	glTranslatef(c_x, c_y, -3.7f);
	glRotatef(cangle, 0.0f, -1.0f, 0.0f);
	Circle();
	glLoadIdentity();

	glTranslatef(l_x, l_y, -3.7f);
	Line();
	glLoadIdentity();
...
void update(void)
{
	...
	t_x = lerp(-1.0f, 0.0f, t);
	t_y = lerp(-1.0f, 0.0f, t);
	c_x = lerp(1.0f, 0.0f, t);
	c_y = lerp(-1.0f, 0.0f, t);
	l_x = lerp(0.0f, 0.0f, t);
	l_y = lerp(1.0f, 0.0f, t);
