...
void printOpenCLDeviceProperties(void)
{
    ...
    else
    {
        ...
        printf("Total  Number Of OpenCL Supporting GPU Devices/ Devices on this system : %d\n", dev_count);
        ...
        for (int i = 0; i < (int)dev_count; i++)
        {
            ...
            clGetDeviceInfo(ocl_device_ids[i], CL_DEVICE_MAX_CONSTAT_BUFFER_SIZE,sizeof(mem_size), &mem_size, NULL);
