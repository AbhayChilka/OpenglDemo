...
//cuda interoperability header files
...
//cuda opengl interoperability releated variables
...
struct cudaGraphicsResource *cudagraphicresource = NULL;
...
int initialize(void)
{
	...
	//code
	int dev_count ;
	//check cuda support amd if supported select default cuda device
	cudaResult = cudaGetDeviceCount(&dev_count);

	if (cudaResult != cudaSuccess)
	{
		fprintf(gpFILE,"cuda get device coun function failed");
		uninitialize();
		exit(0);
	}
	else if (dev_count == 0)
	...
	cudaResult = cudaGraphicsGLRegisterBuffer(&cudagraphicresource, vbo_gpu, cudaGraphicsMapFlagsWriteDiscard);
...
void display(void)
{
	...
	if (bOnGPU == TRUE)
	{
		...
		cudaResult = cudaGraphicsMapResources(1, &cudagraphicresource, 0); 
		if (cudaResult != cudaSuccess)
		{
			fprintf(gpFILE,"cudaGraphicsMapResouces failed");
			uninitialize();
			exit(0);
		}

		//get mapped resource pointer usable in host code
		cudaResult = cudaGraphicsResourceGetMappedPointer((void**)&pPosition, NULL, cudagraphicresource); 
		if (cudaResult != cudaSuccess)
		{
		...
		//launch cuda kernel to write data o retrive host mapped pointer
		launchCudaKernel(pPosition, mesh_width, mesh_width, animationTime);
		if (cudaResult != cudaSuccess)
		{
		...
		//unmap the host usable pointer to read on host
		cudaResult = cudaGraphicsUnmapResources(1, &cudagraphicresource, 0); 
		if (cudaResult != cudaSuccess)
		{
		...
		//re-bind with opengl with buffer object
		glBindBuffer(GL_ARRAY_BUFFER, vbo_gpu);
	}
	else
	{
	...
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				if (k == 0)
				{
					position[i][j][k] = u; //mapped with x
				}
...
void uninitialize(void)
{
	...
	if (vbo_gpu)
	{
		if (cudagraphicresource)
		{
//			cudaGraphicsUnregisterResource(cudagraphicresource) = NULL;
		}
