...
//opencl releated global declaration
...
int initialize(void)
{
	...
	//check opencl support and supported set default 0
	//
	//openl variabl
	...
	{
		fprintf(gpFILE, "clGetPlatformId function failed");
		uninitialize();
		exit(0);
	}
	...
	{
		fprintf(gpFILE, "there are no opencl device");
		uninitialize();
		exit(0);
	}
	else
	{
		...
		{
		fprintf(gpFILE, "memory allocation failed for opecl ids");
		uninitialize();
		exit(0);
		}	

	//fill opencl device arrays
	oclResult = clGetDeviceIds(oclPlatformId, CL_DEVICE_TYPE_GPU, oclDeviceIds, NULL);
	if (oclResult != CL_SUCCESS)
	{
		fprintf(gpFILE, "clGetPlatformId function failed");
		uninitialize();
	...
	//declare opencl cotext properties array
	cl_context_properties oclContextProperties[] = 
												{
													CL_GL_CONTEXT_KHR, (cl_context_properties)wglGetContext(),
													CL_WGL_HDC_KHR , (cl_context_properties)wglGetCuttenDC(),
	...
	if (oclResult != CL_SUCCESS)
	{
		fprintf(gpFILE, "oclContextProperties function failed");
		uninitialize();
		exit(0);
	}
	...
	const char *oclKernelSourceCode = "_kernel void sineWaveKernel(_global float4 *pos , int width , int height , float time)" \
										"{" \
	...
	//from above kernel create opencl program
