...
int initialize(void)
{
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szinfolog != NULL)
			{
				glGetShaderInfoLog(geometryShaderObject, GL_INFO_LOG_LENGTH, NULL, szinfolog);
				fprintf(gpFILE, "GeometryShader Compilation error log : %s\n", szinfolog);
				free(szinfolog);
				szinfolog = NULL;
			}
		}		
	...
	const GLchar* fragmentShaderSourceCode = 
											...
											"void main(void)" \
											"{" \
											"FragColor=vec4(1.0, 1.5, 0.0, 1.0);" \
											"}";
