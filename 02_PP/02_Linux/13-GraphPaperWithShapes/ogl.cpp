...
GLuint mvpMatrixUniform = 0, colorUniform;
...
colorUniform = glGetUniformLocation(shaderProgramObject, "u_Color");
	...
	glBindVertexArray(vao_graph);
	{
		...
		{
			y += 0.05f;
			graph_vertices.push_back(vmath::vec3(-1,y, 0.0f));
			graph_vertices.push_back(vmath::vec3(1,y, 0.0f));
			graph_vertices.push_back(vmath::vec3(-1,-y, 0.0f));
			graph_vertices.push_back(vmath::vec3(1,-y, 0.0f));
	...
	//triangle
	glGenVertexArrays(1, &vao_triange);
	glBindVertexArray(vao_triange);
	{
		glGenBuffers(1, &vbo_position);
		glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
...
//delete vbo of color
	...
	if (vao_triange)
	{
		glDeleteVertexArrays(1, &vao_triange);
		vao_triange = 0;
	}
